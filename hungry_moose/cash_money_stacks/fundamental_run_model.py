...
if __name__ == '__main__':
    ...
    parser.add_argument("-no_feature_column_names",
                        default=['gvkey', 'tic', 'datadate', 'rdq', 'datadate', 'fyearq', 'fqtr',
    ...
    features_column = [x for x in sector_data.columns.values if (x not in no_feature_column_names) and (
                np.issubdtype(sector_data[x].dtype, np.number) and (not np.any(np.isnan(sector_data[x]))))]
