...
class DataBitch:
    ...
    def sc_transform_predictions(self, inverse=False):
        if inverse:
            ...
            self.predictions_future = self.sc_predict.inverse_transform(self.predictions_future.reshape(-1, 1))
        else:
    ...
    def format_for_plot(self, datapoints, columns: list, date_list, train=False, future=False):
        if train:
